...
for k = 1:length(signalTypes)
    ...
    signal = generateSignal(signalTypes(k), time, config.SIGNAL_PARAMETER_1(k), config.SIGNAL_PARAMETER_2(k), config.SIGNAL_PARAMETER_3(k), config.SIGNAL_PARAMETER_4(k), config.SIGNAL_PARAMETER_5(k));
    signals{k} = signal;
...
function signal = generateSignal(signalType, time, mu, sigma, skew1, skew2, kurt)

switch signalType
    
    case 1
        s1 = pearsrnd(mu,sigma,skew1,kurt,floor(length(time)),1);
        s1 = s1(1 : floor(length(time) / 100));
        s1 = resample(s1, 50, 1);
        
        s2 = pearsrnd(mu,sigma,skew2,kurt,floor(length(time)),1);
    ...
    case 2
        s1 = pearsrnd(mu,sigma,skew2,kurt,floor(length(time)),1);
        s1 = resample(s1, 1, 2);
        
        s2 = pearsrnd(mu,sigma,skew1,kurt,floor(length(time)),1);
        s2 = resample(s2, 1, 2);
        signal = [s1;...
            s2];
    case 3
