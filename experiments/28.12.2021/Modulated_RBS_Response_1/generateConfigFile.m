...
for k = 1:length(signalTypes)
    ...
    signal = generateSignal(signalTypes(k), time, config.SIGNAL_PARAMETER_1(k), config.SIGNAL_PARAMETER_2(k), config.SIGNAL_PARAMETER_3(k));
    signals{k} = signal;
...
function signal = generateSignal(signalType, time, frequency, amplitude, frequency2)
...
switch signalType
    ...
    case 4
        ...
        %signal = [zeros(floor(length(time)/2), 1); idinput([floor(length(time)/2), 1], 'prbs', [0 1/frequency], [-amplitude amplitude])];
    case 5
        
        signal = idinput([length(time), 1], 'rbs', [0 1/frequency], [-amplitude amplitude]) .* ((idinput([length(time), 1], 'rbs', [0 1/frequency2], [-amplitude amplitude])+amplitude)/(2 * amplitude));
