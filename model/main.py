...
if __name__ == '__main__':
    ...
    train_dataset, test_dataset = dataset_util.generate_datasets(
        DATA_PATH, sequence_length=SEQUENCE_LENGTH, network_type=NETWORK_TYPE, concat_all=CONCAT_ALL, train_test_split=0.5)
    ...
    optimizer = torch.optim.RMSprop(model_.parameters(), lr=0.005, momentum=0.9)
    iter = 0
    ...
    while True:
        ...
        evaluate(model=model_, device=device, test_loader=test_dataset_loader, criterion = criterion_test, batch_size=BATCH_SIZE)
