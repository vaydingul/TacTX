        ...
        for trial_idx, (train_trial) in enumerate(train_loader):
            ...
            for batch_idx, (data, target) in enumerate(train_trial_loader):
                ...
                pbar.set_description(
                    desc=f'Loss={loss.item():.2f} Epoch = {epoch_idx+1} Trial_idx={trial_idx+1} Batch_idx={batch_idx+1} Accuracy={accuracy:.2f}')
...
def animate_train_evaluate(model, device, train_loader, test_loader, criterion_train, criterion_test, optimizer, batch_size, epoch, num_training):
    ...
    plt.xkcd()
    fig = plt.figure()
    ax_train_loss = fig.add_subplot(221, title='Training Loss', xlabel = 'Epoch', ylabel = 'Loss')
    ax_train_acc = fig.add_subplot(222, title='Training Accuracy', xlabel = 'Epoch', ylabel = 'Accuracy')
    ax_test_loss = fig.add_subplot(223, title='Test Loss', xlabel = 'Epoch', ylabel = 'Loss')
    ax_test_acc = fig.add_subplot(224, title='Test Accuracy', xlabel = 'Epoch', ylabel = 'Accuracy')
    

    train_loss_line = ax_train_loss.plot([0], [0], marker = 'o', c = 'r')
    train_acc_line = ax_train_acc.plot([0], [100], marker = '^', c = 'g')
    test_loss_line = ax_test_loss.plot([0], [0], marker = 'o', c = 'r')
    test_acc_line = ax_test_acc.plot([0], [100], marker = '^', c = 'g')
    ...
    def animate(i):
        ...
        train_loss_line[0].set_ydata(np.hstack([train_loss_line[0].get_ydata(), train_loss.item()]))
        train_loss_line[0].set_xdata(np.hstack([train_loss_line[0].get_xdata(), i * epoch]))

        train_acc_line[0].set_ydata(np.hstack([train_acc_line[0].get_ydata(), train_acc]))
        train_acc_line[0].set_xdata(np.hstack([train_acc_line[0].get_xdata(), i * epoch]))

        test_loss_line[0].set_ydata(np.hstack([test_loss_line[0].get_ydata(), test_loss]))
        test_loss_line[0].set_xdata(np.hstack([test_loss_line[0].get_xdata(), i * epoch]))

        test_acc_line[0].set_ydata(np.hstack([test_acc_line[0].get_ydata(), test_acc]))
        test_acc_line[0].set_xdata(np.hstack([test_acc_line[0].get_xdata(), i * epoch]))

        ax_train_loss.set_xlim(0, i * epoch)
        ax_train_loss.set_ylim(0, max(train_loss_line[0].get_ydata()))
        ax_train_acc.set_xlim(0, i * epoch)
        ax_train_acc.set_ylim(0, 100)
        ax_test_loss.set_xlim(0, i * epoch)
        ax_test_loss.set_ylim(0, max(test_loss_line[0].get_ydata()))
        ax_test_acc.set_xlim(0, i * epoch)
        ax_test_acc.set_ylim(0, 100)

        fig.suptitle('Epoch: {}'.format(i * epoch))
   
        return train_loss_line, train_acc_line, test_loss_line, test_acc_line, ax_train_loss, ax_train_acc, ax_test_loss, ax_test_acc, fig

    animation = FuncAnimation(fig, animate, frames= np.arange(1, num_training + 1), repeat = False)
    fig.tight_layout()
    plt.show()

    return train_loss_line[0].get_ydata(), train_acc_line[0].get_ydata(), test_loss_line[0].get_ydata(), test_acc_line[0].get_ydata()
